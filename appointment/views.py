...
from .serializers import DoctorsAppointmentSerializer , MedicineSerializer
...
def HomeMedicine(request):
    if request.method == 'POST':
        ...
        if 'user_id' in request.GET:
            ...
            services = Medicine.objects.filter(
                user_id=request.GET['user_id']).order_by("-id")

        else:
            services = Medicine.objects.all()
        serializer = MedicineSerializer(services, many=True)
...
@permission_classes((IsAuthenticated,))
@authentication_classes((JWTAuthentication,))
def HomeMedicineDetails(request, pk):
    """
    Retrieve, update or delete a code  DoctorsAppointment.
    """
    try:
        Home_Meedicine = Medicine.objects.get(pk=pk)
    except Home_Meedicine.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = MedicineSerializer(Home_Meedicine)
        return Response(serializer.data)
